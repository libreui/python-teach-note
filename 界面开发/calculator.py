...
tk.Button(window, text="AC", command=clear).grid(row=1, column=0)

tk.Button(window, text="+/-", command=setSymbol).grid(row=1, column=1)
tk.Button(window, text="%", command=setPercent).grid(row=1, column=2)
...
btn0.grid(row=5, column=0, columnspan=2, ipadx=26)

btnDot = tk.Button(window, text=".")
btnDot.config(command=lambda: callback("."))
btnDot.grid(row=5, column=2, ipadx=3)
