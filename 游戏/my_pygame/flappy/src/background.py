...
class Background(Sprite):
    def __init__(self, flappy):
        ...
        # 加载背景图片
        self.image = self.resource.get_background_images()['day']
        self.rect = self.image.get_rect()
        
        # 获取背景图宽度，用于滚动判断
        self.bg_width = self.rect.width
        
        # 第二张背景图的位置（在第一张图右边）
        self.rect2 = self.image.get_rect()
        self.rect2.x = self.bg_width
    ...
    def update(self):
        """更新背景位置，实现无限滚动"""
        # 计算当前帧的滚动速度（基础速度 × 背景滚动系数）
        # 背景作为远景，移动速度较慢（透视效果）
        scroll_speed = self.config.SPEED * self.config.BG_SCROLL_FACTOR
        ...
        if self.rect.x <= -self.bg_width:
            self.rect.x = self.bg_width
        
        # 当第二张图完全移出屏幕左边时，将它放到屏幕最右边
        if self.rect2.x <= -self.bg_width:
            self.rect2.x = self.bg_width
